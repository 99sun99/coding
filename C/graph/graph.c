...
int graph_ins_vertex(Graph *graph, const void *data)
{
  ...
  if ((retval = list_ins_next(&graph->adjlists, list_tail(&graph->adjlists), adjlist)) != 0) {
	  //insert adjlist next to tail， tail contains next listElm tail
	  //void 可以直接传入 其他type的参数
    return retval;
...
int graph_ins_edge(Graph *graph, const void *data1, const void *data2)
{
  ...
  /// 不允许插入顶点不在图中的边 a to b  data1 to data2 , data1 last data2 first
  ...
  /// 将顶点2插入到顶点1的邻接表  element(data1)->adjacent  to 的一方放入邻接表
