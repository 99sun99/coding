...
#include "D:\trainning_course\C\graph\dfs.h"
#include "D:\trainning_course\C\graph\dfs.c"
...
{
  /// 判断任务是否匹配 match return 1 and then return back because we don't want to add an
	//element that already exist
...
{
  Graph       graph;
  DfsVertex   *task, task1, *task2,*tmp;
  List        list;
  ListElmt    *element;
  char        data1[STRSIZ];
  int         i;
  ...
  /// 插入任务依赖关系
  if ((tmp = (DfsVertex *)malloc(sizeof(DfsVertex))) == NULL) return 1;
  //task1.data = data1;
	tmp->data = data1;
  ...
  strcpy((char *)tmp->data, "a");
  strcpy((char *)task2->data, "b");
  fprintf(stdout, "Inserting edge %s to %s\n", (char *)tmp->data, (char *)task2->data);

  //task1 的邻接表变动就不能传指针？
  if (graph_ins_edge(&graph, tmp, task2) != 0) return 1;
