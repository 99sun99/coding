...
#include "D:\trainning_course\C\graph\graph.h"
#include "D:\trainning_course\C\graph\graph.c"
#include "D:\trainning_course\C\graph\list.h"
#include "D:\trainning_course\C\graph\list.c"
#include "D:\trainning_course\C\graph\set.h"
#include "D:\trainning_course\C\graph\set.c"
/// 定义字符串长度
#define     STRSIZ      2
...
static void print_graph(const Graph *graph)
{
  ...
  i = 0;
  element = list_head(&graph_adjlists(graph)); //list's head

  while (i < list_size(&graph_adjlists(graph))) {

    fprintf(stdout, "--> graph[%03d]=%s: ", i, (char *)((AdjList *)list_data(element))->vertex); //cast adjlist

    j = 0;
    adjacent = &((AdjList *)list_data(element))->adjacent;  //cast adjacent set head's adjacent
    member = list_head(adjacent);  //adjacent's head (still a list, contains sets of vertices)
...
{
  /// 字符串对比函数
  return !strcmp((const char *)str1, (const char *)str2); //if not equal return 1. 
}
...
int main(int argc, char **argv)
{
  ...
  if (graph_ins_vertex(&graph, data) != 0) return 1;  // adjList vertex is stored as a list 
