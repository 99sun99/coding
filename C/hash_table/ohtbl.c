               ...
               void (*destroy)(void *data))
{
  ...
  /// 设置哨兵元素
  htbl->vacated = &vacated;  //char
 // printf("what is vacated? %d\n", &vacated);
  /// 封装函数进 hash 表
  htbl->h1 = h1;
  htbl->h2 = h2;
  htbl->match = match;
...
int ohtbl_insert(OHTbl *htbl, const void *data)
{
  ...
  for (i = 0; i < htbl->positions; i++) {
    ...
    position = (htbl->h1(data) + (i * htbl->h2(data))) % htbl->positions;//mainly different
  ...
  for (i = 0; i < htbl->positions; i++) {
    ...
    if (htbl->table[position] == htbl->vacated) {
      ...
      *data = htbl->table[position];
      htbl->table[position] = htbl->vacated;//remove
      htbl->size--;
      return 0;
